...
pub fn to_rust_func(item: &ItemFn, implm: Option<(&Type, &Ident)>) -> DResult<RustFunction> {
        ...
        Type::Path(ref path) => {
            ...
            match path_name.as_str() {
                ...
                "Option" => {
                    if let PathArguments::AngleBracketed(ref inner) = seg0.arguments {
                        if let GenericArgument::Type(ref inner_ty) = inner.args[0] {
                            ...
                            return Err(syn::Error::new(
                                Span::call_site(),
                                "Result or option should not have lifetime",
                            )
                            .into());
                        }
                    } else {
                ...
                | "u128" | "usize" | "f32" | "f64" | "bool" | "char" => {
                    Ok(RustTypes::Primitive(path_name.to_owned()))
                }
                "String" | "str" => Ok(RustTypes::String),
                _ => Ok(RustTypes::Ptr(path_name.to_owned())),
            }
        }
        Type::Ptr(v) => Ok(RustTypes::Ptr(v.elem.to_token_stream().to_string())),
